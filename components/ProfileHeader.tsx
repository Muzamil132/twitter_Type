import { ChatIcon, DotsHorizontalIcon, InboxIcon, InboxInIcon } from '@heroicons/react/solid'
...
const ProfileHeader = ({name,profileImage}:IProps) => {
  ...
  const {query:{profileId},asPath}  =useRouter()
  ...
  const [isFollowing] = useFollowHook(profileId1.toString(),currentId.toString())

 
    const imageUrl= "https://cdn.pixabay.com/photo/2019/09/26/18/57/wasp-4506782_960_720.jpg"
    const image="https://cdn.pixabay.com/photo/2022/09/13/11/29/girl-7451711_960_720.jpg"
    ...
    const routeToChat=()=>{
        ...
        var conversationId = currentId > profileId1 ? `${currentId.toString() + profileId1}` : `${profileId1 + currentId.toString()}`;
    ...
    const followHim=(e:any)=>{
        ...
        addFollower(e,isFollowing,profileId1.toString(),currentId.toString(),username.toString(),userImg.toString())
        ...
        <div className='flex justify-between  mt-4 first-line sm:h-12  '>
            {
                profileNavItem.map((item:any,id:any)=>(
                           <Link key={id} href={item.href} className='hover:bg-[#19181a] cursor-pointer' >
